...
layer make_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes, int max_boxes)
{
	...
	//printf("mask value: %d, %d, %d", l.mask[0], l.mask[1], l.mask[2]);
...
{
    int n;
	//if the delta[index] has been assigned already, change the value with (1 - output[index + stride*class_id])
    if (delta[index]){
	...
	if (focal_loss) {
		...
		// default, no longer use softmaxloss to calc the prob and the loss
		for (n = 0; n < classes; ++n) {
			delta[index + stride*n] = ((n == class_id) ? 1 : 0) - output[index + stride*n];
...
void forward_yolo_layer(const layer l, network_state state)
{
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
					...
					//if the iou is big enough, then the loss of noobj is assigned to zero
					//ignore_thresh and truth thresh only used in the calcing of noobj_loss
		...
		for (truth_count = 0; truth_count < l.max_boxes; ++truth_count)
		{
			box truth = float_to_box_stride(state.truth + truth_count*(4 + 1) + b*l.truths, 1);
			if (!truth.x) break;
		}
		printf("in batch: count t is %d\n", truth_count);
    ...
    *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2)/l.batch;
	fcount = (float)count;
	fclasscount = (float)class_count;
	printf("Region %d Avg IOU: %f, Class: %f, Obj: %f, No Obj: %f, .5R: %f, .75R: %f,  count: %d\n", state.index, avg_iou / fcount, avg_cat / fclasscount, avg_obj / fcount, avg_anyobj / (l.w*l.h*l.n*l.batch), recall / fcount, recall75 / fcount, count);
}
