...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, int use_flip, float jitter, float hue, float saturation, float exposure, int small_object)
{
    ...
    for(i = 0; i < n; ++i){
		...
		cvSaveImage("D:/Code_local/darknet_yolov3/x64/Release/debug_img/debug_image.jpg", src, NULL);
		int oh = src->height;
		int ow = src->width;
        ...
        int truthcount = fill_truth_detection(filename, boxes, d.y.vals[i], classes, flip, dx, dy, 1./sx, 1./sy, small_object, w, h);
		printf("truth box count is %d\n", truthcount);
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, int use_flip, float jitter, float hue, float saturation, float exposure, int small_object)
{
	...
	for (i = 0; i < n; ++i) {
		...
		int truthcount  = fill_truth_detection(random_paths[i], boxes, d.y.vals[i], classes, flip, dx, dy, 1. / sx, 1. / sy, small_object, w, h);
