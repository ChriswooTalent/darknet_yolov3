...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, int dont_show)
{
    ...
    while(1){
		...
		int nboxes = 0;
		//predict, the differences between yolov2 and yolov3 on predicting is that yolov3 will get results of 3 yolo_layers an then do nms, yolov2 has only one detection layer
